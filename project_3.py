...
# recursion limit is increased for large graphs
...
def find_articulation_points(graph, n):    # all articulation points in O(n + m) time are found using DFS
    ...
    def dfs(u):
        ...
        for v in graph[u]:
            if discovery[v] == -1:  # Unvisited
                ...
                # articulation point conditions are checked
    ...
    # DFS from all unvisited vertices are run
    for i in range(n):
        if discovery[i] == -1:
            dfs(i)
...
def is_biconnected(graph, n):    #to check if graph is biconnected (no articulation points)
    if n < 2:
        return True
    articulation_points = find_articulation_points(graph, n)
    return len(articulation_points) == 0

def generate_random_connected_graph(n, m):    # to generate a random connected graph with n vertices and m edges
    ...
    # Creates a spanning tree for connectivity
...
def measure_runtime(n, m, num_trials=30):    #M to masure average runtime for finding articulation points
    # Generate graph once
...
def run_experiments():    # to run experiments for various graph sizes
...
def create_output_table(results, C0, C1):    #to create and display the output table
    print("\n" + "=" * 110)
...
def plot_results(results, C0, C1):    #to create the comparison plot
